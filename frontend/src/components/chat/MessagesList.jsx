...
const DateDivider = ({ date }) => {
  ...
  return (
    <div className="flex items-center gap-3 my-4 px-3 sm:px-4">
      <div className="flex-1 h-px bg-chat-border/30" />
      <span className="text-[11px] sm:text-xs text-chat-secondary bg-chat-bg px-2 py-0.5 rounded-full border border-chat-border/20 whitespace-nowrap">
        {label}
      </span>
      <div className="flex-1 h-px bg-chat-border/30" />
    </div>
...
const TypingIndicator = ({ typingUsers }) => {
  ...
  return (
    <div className="flex items-end gap-1.5 mb-2 px-3 sm:px-4">
...
const MessagesList = () => {
  ...
  const handleScroll = useCallback(() => {
    ...
    if (!container || isLoadingMore.current || !hasMore || !activeChat?._id)
      return;
  ...
  }, [hasMore, activeChat?._id, dispatch]);
  ...
  return (
    ...
    >
      ...
      {!loading && messages.length === 0 && (
        <div className="flex flex-col items-center justify-center h-full text-chat-secondary text-center px-6">
          <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-chat-input flex items-center justify-center mb-3">
            <svg
