  ...
  return (
    <div className="flex items-center justify-between px-4 py-3 bg-chat-header border-b border-chat-border/20">
      <div className="flex items-center gap-3 flex-1 min-w-0">
        {/* Mobile back button */}
        <button
          className="md:hidden btn-ghost p-1.5 rounded-full -ml-1"
        ...
        >
          ...
          <div className="min-w-0">
            <h2 className="font-semibold text-chat-text text-sm truncate">{chatName}</h2>
            <p className={`text-xs truncate ${isTyping ? 'text-primary-500' : 'text-chat-secondary'}`}>
              {getSubtitle()}
            </p>
          </div>
        </button>
      ...
      <div className="flex items-center gap-1">
        <button className="btn-ghost p-2 rounded-full" title="Search">
          <svg className="w-5 h-5 text-chat-secondary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
