...
const NewChatModal = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => dispatch(toggleNewChat())} />
      <div className="relative bg-chat-header rounded-2xl w-full max-w-md shadow-2xl border border-chat-border/30 fade-in">
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-chat-border/20">
          <h2 className="text-base font-semibold text-chat-text flex-1">New Chat</h2>
          <button onClick={() => dispatch(toggleNewChat())} className="btn-ghost p-1.5 rounded-full">
        ...
        <div className="max-h-80 overflow-y-auto">
          ...
          {!loading && !search && (
            <div className="text-center py-8 text-chat-secondary">
              ...
              <p className="text-sm">Search for people to chat with</p>
            </div>
          )}
          ...
          {users.map((u) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="font-medium text-chat-text text-sm">{u.name}</p>
                <p className="text-xs text-chat-secondary truncate">{u.email}</p>
              </div>
              {u.status === 'online' && (
