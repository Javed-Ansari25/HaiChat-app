...
const ChatInfoPanel = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-chat-sidebar border-l border-chat-border/20 w-72">
      {/* Header */}
      <div className="flex items-center gap-3 px-4 py-3 bg-chat-header border-b border-chat-border/20">
        <button onClick={() => dispatch(setChatInfo(false))} className="btn-ghost p-1.5 rounded-full">
          <svg className="w-5 h-5 text-chat-secondary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
          </svg>
        </button>
        <h2 className="font-semibold text-chat-text text-sm">
          {activeChat.isGroupChat ? 'Group Info' : 'Contact Info'}
        </h2>
      </div>

      {/* Profile */}
      <div className="flex flex-col items-center py-6 bg-gradient-to-b from-chat-header/50 to-transparent border-b border-chat-border/20">
        <Avatar src={chatAvatar} name={chatName} size="xl" showOnline={!activeChat.isGroupChat} isOnline={isOnline} />
        <h3 className="text-base font-semibold text-chat-text mt-3">{chatName}</h3>
        {!activeChat.isGroupChat && (
          <p className={`text-xs mt-1 ${isOnline ? 'text-primary-500' : 'text-chat-secondary'}`}>
            {isOnline ? 'online' : partner?.lastSeen ? `last seen ${formatLastSeen(partner.lastSeen)}` : 'offline'}
          </p>
        )}
        {activeChat.isGroupChat && (
      ...
      <div className="flex-1 overflow-y-auto">
        {!activeChat.isGroupChat && partner?.bio && (
          <div className="p-4 border-b border-chat-border/20">
            ...
            <p className="text-sm text-chat-text">{partner.bio}</p>
          </div>
        )}
          ...
          <>
            {activeChat.groupDescription && (
              <div className="p-4 border-b border-chat-border/20">
                ...
                <p className="text-sm text-chat-text">{activeChat.groupDescription}</p>
              </div>
            )}
              ...
              <div className="space-y-2">
                {activeChat.participants?.map((p) => (
                  <div key={p._id} className="flex items-center gap-3">
                    <Avatar src={p.avatar} name={p.name} size="sm" showOnline isOnline={p.status === 'online'} />
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-chat-secondary">
                        {activeChat.groupAdmin === p._id || activeChat.groupAdmin?._id === p._id ? 'Admin' : 'Member'}
                      </p>
                    </div>
                    {p.status === 'online' && (
        ...
        <div className="p-4 border-t border-chat-border/20">
          <button
            ...
            className="w-full flex items-center gap-2 py-2.5 px-3 rounded-lg bg-chat-input hover:bg-chat-hover transition-colors text-sm text-chat-text"
          >
